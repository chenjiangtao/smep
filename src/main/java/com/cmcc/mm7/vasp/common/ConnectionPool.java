...
public class ConnectionPool implements Runnable {
	...
	public List<ConnectionWrap> ClientList;
	...
	private void init() {
		...
		String xml = Mm7Config.getConnConfig();
		if (!xml.equals("")) {
			confManager.load(xml);
	...
	public synchronized ConnectionWrap getConnWrap() {
		if (ClientList == null) {
			...
			for (int i = 0; i < ClientList.size(); i++) {
				ConnectionWrap conn = ClientList.get(i);
			...
			if (ClientList.size() < MaxCount) {
				...
				if (ClientList.isEmpty() == true) {
					...
					ConnectionWrap conn = ClientList.get(ClientList.size() - step);
					conn.setFree(false);
	...
	public void addURL(int count) {
		...
		if (ClientList == null)
			ClientList = new ArrayList<ConnectionWrap>(count);
		try {
