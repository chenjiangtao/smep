...
public class SOAPDecoder {
		...
		try {
			if (OutputMessage == null) {
				...
				if (index1 > 0) {
					...
					xmlend = message.indexOf("--".concat(strboundary), xmlbeg + 1);
					if (xmlend > 0)
	...
	private MM7DeliverReq parseAttachment(ByteArrayOutputStream content, MM7DeliverReq deliverReq) {
		...
		int index = 0;
		byte[] bBoundary = ("--".concat(boundary)).getBytes();
		int m = 0;
			...
			if (type_index > 0) {
				...
				if (attindex > 0) {
					...
					if (encodingindex >= 0) {
						...
						String EncodingType = attachment.toString().substring(encodingindex + strEncoding.length() + 1, endEncoding).trim();
	...
	private MM7RSReq parseXML(String xmlContent) {
		...
		if (index1 > 0) {
			xml = xmlContent.substring(0, index1).concat("encoding=\"GB2312\"").concat(xmlContent.substring(index1 + "encoding=\"UTF-8\"".length()));
			xmlContent = xml;
		}
						...
						for (int j = 0; j < recsize; j++) {
							...
							if (rec.getName().equals("To")) {
								...
								for (int m = 0; m < tosize; m++) {
									...
									if (ele.getName().equals("Number") || ele.getName().equals("RFC2822Address") || ele.getName().equals("ShortCode"))
