...
public class MM7Receiver implements MM7AbstractReceiver {
	...
	//private final SimpleDateFormat Recordsdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	/*
	 * private void setSameMinuteTime(long time) { SameMinuteTime = time; }
	 * 
	 * private long getSameMinuteTime() { return SameMinuteTime; }
	 * 
	 * private void setSameMMSCID(int mmscid) { SameMMSCID = mmscid; }
	 * 
	 * private int getSameMMSCID() { return SameMMSCID; }
	 */
	...
	*/
		...
		try {
			...
			while (true) {
				...
				if (bHead == false) {
					...
					length = Integer.parseInt(baos.toString().substring((index1+ MMConstants.CONTENT_LENGTH.length() + 1), index2).trim());
	...
	public void send(Socket socket, MM7VASPRes mm7VASPRes, boolean longLink) {
		...
		try {
			...
			beforAuth.append("Content-Type: text/xml;charset=".concat(Config.getCharSet()).concat("\r\n"));
			// 设置鉴权
			byte[] bcontent = getContent(mm7VASPRes);
			// 设置消息体长度
			afterAuth.append("Content-Length: ".concat(bcontent.length + "\r\n"));
			afterAuth.append("\r\n");
			sb.append(beforAuth);
			sb.append(afterAuth);
	...
	private byte[] encodeMessage(MM7VASPRes mm7VASPRes) {
		...
		if (mm7VASPRes.isTransactionIDExist()) //
			sb.append(String.format("<mm7:TransactionID xmlns:mm7=\"http://www.3gpp.org/ftp/Specs/archive/23_series/23.140/schema/REL-6-MM7-1-0\">%s</mm7:TransactionID>", mm7VASPRes.getTransactionID()));
		else
		...
		if (mm7VASPRes instanceof MM7DeliverRes) {
			...
			if (deliverRes.isMM7VersionExist()) {
				sb.append(String.format("<MM7Version>%s</MM7Version>", deliverRes.getMM7Version()));
			} else {
				System.err.println("MM7Version 不许为空！");
			}

			if (deliverRes.isServiceCodeExist()) {
				sb.append(String.format("<ServiceCode>%s</ServiceCode>", deliverRes.getServiceCode()));
			}

			sb.append("<Status>");
			if (deliverRes.isStatusCodeExist()) {
				sb.append(String.format("<StatusCode>%s</StatusCode>", deliverRes.getStatusCode()));
			} else {
				System.err.println("StatusCode 不许为空");
			}

			if (deliverRes.isStatusTextExist()) {
				sb.append(String.format("<StatusText>%s</StatusText>", deliverRes.getStatusText()));
			}

			if (deliverRes.isStatusDetailExist()) {
				sb.append(String.format("<Details>%s</Details>", deliverRes.getStatusDetail()));
			}
			sb.append("</Status>");
			...
			if (deliveryReportRes.isMM7VersionExist()) {
				sb.append(String.format("<MM7Version>%s</MM7Version>", deliveryReportRes.getMM7Version()));
			} else {
				System.err.println("MM7Version 不许为空！");
			}
			...
			if (deliveryReportRes.isStatusCodeExist()) {
				sb.append(String.format("<StatusCode>%s</StatusCode>", deliveryReportRes.getStatusCode()));
			} else {
				System.err.println("StatusCode 不许为空");
			}

			if (deliveryReportRes.isStatusTextExist()) {
				sb.append(String.format("<StatusText>%s</StatusText>", deliveryReportRes.getStatusText()));
			}

			if (deliveryReportRes.isStatusDetailExist()) {
				sb.append(String.format("<Details>%s</Details>", deliveryReportRes.getStatusDetail()));
			}
			sb.append("</Status>");
			...
			if (readReplyRes.isMM7VersionExist()) {
				sb.append(String.format("<MM7Version>%s</MM7Version>", readReplyRes.getMM7Version()));
			} else {
				System.err.println("MM7Version 不许为空！");
			}
			...
			if (readReplyRes.isStatusCodeExist()) {
				sb.append(String.format("<StatusCode>%s</StatusCode>", readReplyRes.getStatusCode()));
			} else {
				System.err.println("StatusCode 不许为空");
			}

			if (readReplyRes.isStatusTextExist()) {
				sb.append(String.format("<StatusText>%s</StatusText>", readReplyRes.getStatusText()));
			}

			if (readReplyRes.isStatusDetailExist()) {
				sb.append(String.format("<Details>%s</Details>", readReplyRes.getStatusDetail()));
			}
