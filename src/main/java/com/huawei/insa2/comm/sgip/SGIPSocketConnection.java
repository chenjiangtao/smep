...
public abstract class SGIPSocketConnection extends PLayer {
	...
	*/
	...
	protected void initSMGConnect(Args args) {
		resource = getResource();
		initResource();
		error = NOT_INIT;
		setAttributesOfRemote(args);
		
		class RemoteReceiveThread extends WatchThread {
		...
		receiveThread = new RemoteReceiveThread();
		receiveThread.start();
	}
	
	protected void initSPConnect(Args args, Socket socket) {
		resource = getResource();
		initResource();
		error = NOT_INIT;
		if (socket != null) {
			...
			try {
				...
				setError(CONNECT_ERROR.concat(explain(ex)));
			}
			
			if (args != null)
				setAttributesOfLocal(args);

			class LocalReceiveThread extends WatchThread {
				...
				public LocalReceiveThread() {
					super(name + "-receive");
				}
			}

			receiveThread = new LocalReceiveThread();
			receiveThread.start();
		}
	}
	...
	public void setAttributesOfRemote(Args args) {
		...
		int oldPort = port;
		/*
		 * String oldLocalHost = localHost; int oldLocalPort = localPort;
		 */
	...
	public void setAttributesOfLocal(Args args) {
		if (name != null && name.equals(host.concat(":").concat(String.valueOf(port))))
			name = null;
	...
	public String toString() {
		return String.valueOf(String.valueOf((new StringBuffer(
				"PShortConnection:")).append(name).append('(').append(host)
