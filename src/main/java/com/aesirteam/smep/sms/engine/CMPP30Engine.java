...
@Component("cmpp30Engine")
public class CMPP30Engine extends BaseEngine<CMPPMessage> {
	protected final static Logger logger = LoggerFactory.getLogger(CMPP30Engine.class);
	protected static String className = CMPP30Engine.class.getSimpleName();
	protected HwSMProxy30 hwSMProxy;
	...
	protected void consumeTask() {
		...
		CMPP30SubmitRepMessage rsp = null;
		try {
			rsp = (CMPP30SubmitRepMessage) hwSMProxy.send(message);
		} catch (IOException e) {
			logger.error(String.format("[%s SMProxy30] Connection fail!!! [%d]", Thread.currentThread().getId(), cache.size()));
			//发送时网关连接异常
			producer(message);
			return;
		}
		...
		//System.out.println("cmpp30: " + msgMtLog.getSeqNo() + "\t" + msgMtLog.getMsgId() + "\t" + msgMtLog.getDest_Terminal_Id() + "\t" + msgMtLog.getMsg_Content());
		if (0 == rsp.getResult()) {
	...
	protected class HwSMProxy30 extends SMProxy30 {

		public HwSMProxy30(Args args) {
			super(args);
	...
	protected boolean initSMProxy() {
		try {	
			hwSMProxy = new HwSMProxy30(args);
			return true;
	...
	public CMPPMessage getCMPPMessage(ReqSmsMessage req) throws UnsupportedEncodingException {
		...
		CMPPMessage message = new CMPP30SubmitMessage(pk_Total, pk_Number,
				registered_Delivery, msg_Level, service_Id, fee_UserType,
				fee_Terminal_Id, fee_Terminal_Type, tp_Pid, tp_Udhi, msg_Fmt,
